...
CAPS = {
 ...
 "nli": "wnioskowanie logiczne między przesłanką a hipotezą. OSTRE definicje: 'wynikanie' TYLKO gdy hipoteza wynika z przesłanki z konieczności (nie 'prawdopodobnie'); 'sprzeczność' TYLKO gdy nie mogą być prawdziwe jednocześnie; KAŻDY inny przypadek (możliwe ale niepewne, brak związku, dodatkowe założenia) to 'neutralna'. WYMYŚL różnorodne pary przesłanka-hipoteza z REALISTYCZNYM rozkładem: około 55-60% neutralnych (w tym subtelne pary, które KUSZĄ by powiedzieć wynikanie, ale wymagają dodatkowych założeń), reszta po równo. W odpowiedzi etykieta + jedno zdanie uzasadnienia.",
