...
Buduje indeks n-gramów słów (domyślnie 8) ze WSZYSTKICH zbiorów ewaluacyjnych:
  ...
  - slayer-data/llmzszl_R_eval.jsonl          (próbka LLMzSzŁ)
...
EVAL_SOURCES = [
    "runs/test_atoms.txt",
    "slayer-data/llmzszl_R_eval.jsonl",
...
GEN_DEFAULT = [
    "slayer-data/distill/distill_pl.jsonl",
    "slayer-data/knowledge/entigraph_pl_focus.jsonl",
...
def audit_file(path, idx, n, strip):
    rows, hits = [], []
    total = 0
    for ln in open(path, encoding="utf-8"):
        ln = ln.rstrip("\n")
...
def main():
    ...
    print(f"[decon] indeks: {len(idx)} {a.ngram}-gramów z evali")
    for s, c in per_src.items():
        print(f"  {s}: {c}")

    results = []
    for p in paths:
        r = audit_file(p, idx, a.ngram, a.strip)
    ...
    json.dump({"ngram": a.ngram, "index_size": len(idx), "eval_sources": per_src,
               "results": results}, open(a.report, "w"), ensure_ascii=False, indent=2)
    print(f"[decon] raport -> {a.report}")
